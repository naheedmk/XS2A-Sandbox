...
export class AccountService {
  ...
  listAccounts(page: number = 0, size: number = 25, queryParam: string = ''): Observable<{accounts: Account[], totalElements: number}> {
    return this.http.get<PaginationResponse<Account[]>>
    (`${this.url}/accounts/page?page=${page}&size=${size}&queryParam=${queryParam}`).pipe(
      map((resp) => {
        return {
