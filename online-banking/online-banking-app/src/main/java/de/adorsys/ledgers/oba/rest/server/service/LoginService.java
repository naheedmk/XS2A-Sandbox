...
public class LoginService {
    ...
    public ResponseEntity<PaymentAuthorisationResponse> login(String encryptedPaymentId,
        ...
        if (AuthUtils.isSuccessfulAuthorisation(authoriseForConsent)) {
            return evaluateScaMethodsAndGetResponse(response, workflow);
        } else {
    ...
    private ResponseEntity<PaymentAuthorisationResponse> evaluateScaMethodsAndGetResponse(HttpServletResponse response, PaymentWorkflow workflow){
        String psuId = AuthUtils.getPsuId(workflow.getBearerToken());
        try {
        ...
        return getAuthorisationResponseBasedOnScaStatus(response, workflow);
    }

    @NotNull
    private ResponseEntity<PaymentAuthorisationResponse> getAuthorisationResponseBasedOnScaStatus(HttpServletResponse response, PaymentWorkflow workflow) {
